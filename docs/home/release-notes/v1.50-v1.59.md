...
Background migrations, running during `core` container startup, may last up to tens of minutes.
Increase `failureThreshold` field value for core container and/or
helm `--timeout` option if ODM is not able to start in the allocated time.
